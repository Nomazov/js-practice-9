...
function findLastEvenNumber(arr) {
    ...
    const lastNumber = arr.findLast(num => num % 2 === 0);

    return lastNumber !== undefined ? lastNumber : "Парних чисел не знайдено', якщо таке число не знайдено";
...
function getArrayKeys(arr) {
    ...
    const itter = arr.keys();

    return Array.from(itter);
    // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function getPositiveArrayValues(arr) {
    ...
    const itter = Array.from(obj);

    return itter.filter(item => item > 0);
    // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function removeAndDouble(arr, startIdx) {
    ...
    const arr2 = arr.splice(startIdx);
...
function flattenSquareAndFilter(arr, threshold) {
    ...
    return arr.flat().map(item => item ** 2).filter(item => item > threshold);
    // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function getLastElementsSumAndPrint(arr, n) {
    ...
    const summ = last.reduceRight((total, current) => total + current, 0);
...
function capitalizeWords(arr) {
    ...
    const capitalizedArray = arr.map(([firstLetter, ...rest]) => firstLetter.toUpperCase() + rest.join(''));
